...
def test_rows_empty_data() -> None:
    empty_data = Hours([])
    assert rows(empty_data) == "Data from csv file"


def test_rows_with_single_entry_data(sample_hour_entry_1: Entry) -> None:
    data = Hours([sample_hour_entry_1])
    expected_output = """
Data from csv file
...
def test_rows_with_multiple_entries_data(
    sample_hour_entry_1: Entry, sample_hour_entry_2: Entry, sample_hour_entry_3: Entry
) -> None:
    data = Hours([sample_hour_entry_1, sample_hour_entry_2, sample_hour_entry_3])
    expected_output = """
Data from csv file
1: 2024-05-01T02:00:00.000+0000 (01.05.2024), 3600 (1), 'JKL-123', 'Sample Issue'
...
def test_hours_per_day_empty_data() -> None:
    empty_data = Hours([])
    assert hours_per_day(empty_data) == "Hours per date"


def test_hours_per_day_with_single_entry_data(sample_hour_entry_1: Entry) -> None:
    data = Hours([sample_hour_entry_1])
    expected_output = """
Hours per date
2024-05-01: 1.0
...
def test_hours_per_ticket_empty_data() -> None:
    empty_data = Hours([])
    assert hours_per_ticket(empty_data) == "Hours per ticket"


def test_hours_per_ticket_with_single_entry_data(sample_hour_entry_1: Entry) -> None:
    data = Hours([sample_hour_entry_1])
    expected_output = """
Hours per ticket
JKL-123: 1.0
